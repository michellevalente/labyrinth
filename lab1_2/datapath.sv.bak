...
generate
        ...
        begin
            memory_8b_1entry dist(.clk(clk),.write(wenable_dist[counter]),.in(in_dist[counter]),.out(out_dist[counter]),.address(address));
        end
    end
endgenerate
...
//TO DELETE: ONLY TO CHECK IF MEMORIES ARE BEING INITIALIZED CORRECTLY
